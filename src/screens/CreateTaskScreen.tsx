...
import { CreateTaskForm, Project, User, Task } from '../types';
...
const CreateTaskScreen = () => {
  ...
  const preselectedProjectId = (route.params as { projectId?: string } | undefined)?.projectId;
  ...
  const validateForm = () => {
    ...
    // Due date validation (optional but if provided, should be in the future)
    if (formData.dueDate) {
        ...
        Alert.alert('Success', 'Task created successfully!', [
          {
            ...
            onPress: () => {
              if (result.data) {
                // @ts-ignore
                navigation.navigate('TaskDetails', { taskId: result.data.id });
              } else {
                navigation.goBack();
              }
            },
  ...
  const renderProjectPicker = () => (
    <View style={styles.section}>
      ...
      <TouchableOpacity
        style={[
          styles.pickerButton,
          errors.projectId ? styles.pickerButtonError : null,
        ]}
        ...
        >
          {formData.assigneeId ? (
            <View style={styles.selectedItem}>
              <Avatar
                name={`${
                  availableAssignees.find(a => a.id === formData.assigneeId)
                    ?.firstName
                } ${
                  availableAssignees.find(a => a.id === formData.assigneeId)
                    ?.lastName
      ...
      <View style={styles.section}>
        ...
        >
          {selectedPriorityInfo && (
            <View style={styles.selectedItem}>
              <Badge
                label={selectedPriorityInfo.label}
                size="sm"
                customColor={selectedPriorityInfo.color}
              />
            </View>
          )}
          <Text style={styles.pickerArrow}>▼</Text>
          ...
          <View style={styles.pickerList}>
            {priorities.map(priority => (
              <TouchableOpacity
                ...
                style={styles.pickerItem}
                onPress={() =>
                  selectPriority(priority.value as CreateTaskForm['priority'])
                }
              >
                <Badge
